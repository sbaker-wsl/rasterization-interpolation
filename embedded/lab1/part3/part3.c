...
/* note: most of this is code directly from the University Project, so I did not edit out credit to it as the only addition I made was the functionality described
in the lab (make it so each time KEY is pressed it increments unless it is 9 which then goes back to 0 */
...
{
    *LEDR_ptr = *LEDR_ptr + 1;
    if ((count = *LEDR_ptr & 0x0F) == 10) {
        *LEDR_ptr = *LEDR_ptr - 10;
        count = count - 10;
...
static int __init initialize_pushbutton_handler(void)
{
   ...
   *LEDR_ptr = 0x80;                  // turn on the leftmost light
